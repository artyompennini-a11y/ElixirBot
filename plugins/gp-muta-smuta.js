...
let handler = async (m, { conn, text, command, isOwner, isROwner }) => {
  try {
    ...
    let messaggio = ''
    if (isMute) {
      messaggio = `┌─── 「 🔇 *ᴜᴛᴇɴᴛᴇ ᴍᴜᴛᴀᴛᴏ* 」 ───┐\n` +
                  `│\n` +
                  `│ 👤 *ᴛᴀʀɢᴇᴛ:* ${targetTag}\n` +
                  `│ 👑 *ᴇꜱᴇɢᴜɪᴛᴏ ᴅᴀ:* ${executorTag}\n` +
                  `│ ⏳ *ᴅᴜʀᴀᴛᴀ:* \`${duration?.label || 'Permanente'}\`\n` +
                  `│\n` +
                  `├───────────────────────────\n` +
                  `│\n` +
                  `│ ℹ️ _I messaggi inviati dall'utente verranno_\n` +
                  `│ _intercettati ed eliminati automaticamente._\n` +
                  `│\n` +
                  `└───────────────────────────┘\n` +
                  `> *THE PUNISHER-BOT*`
    } else {
      messaggio = `┌─── 「 🔊 *ᴍᴜᴛɪɴɢ ʀᴇᴠᴏᴄᴀᴛᴏ* 」 ───┐\n` +
                  `│\n` +
                  `│ 👤 *ᴛᴀʀɢᴇᴛ:* ${targetTag}\n` +
                  `│ 👑 *ꜱʙʟᴏᴄᴄᴀᴛᴏ ᴅᴀ:* ${executorTag}\n` +
                  `│\n` +
                  `├───────────────────────────\n` +
                  `│\n` +
                  `│ ℹ Senza restrizioni. L'utente può riprendere_\n` +
                  `│ _regolarmente l'attività all'interno della chat._\n` +
                  `│\n` +
                  `└───────────────────────────┘\n` +
                  `> *THE PUNISHER-BOT*`
    }
