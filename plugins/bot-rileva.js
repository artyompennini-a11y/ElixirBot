...
export async function before(m, { conn, groupMetadata }) {
  ...
  if (!gm) {
    try { gm = await conn.groupMetadata(m.chat).catch(() => null) } catch {}
  }
  if (!gm) return
  ...
  if (sender && typeof sender === 'string' && sender.endsWith('@lid')) {
      ...
      const participant = gm.participants.find(p => p.id && (p.id.split('@')[0] === lidNumber));
  ...
  const nomegp = gm.subject || 'vare ✧ bot';
  const am = {
